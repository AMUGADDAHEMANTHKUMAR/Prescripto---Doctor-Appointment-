          ...
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && !item.payment && (
                <button
                  type='button'
                  onClick={() => navigate(`/payment?amount=${encodeURIComponent(item.amount)}&appointmentId=${encodeURIComponent(item._id)}`)}
              ...
              {!item.cancelled && item.payment && (
                <button type='button' disabled className='text-sm text-green-700 sm:min-w-48 py-2 border border-green-600 rounded bg-green-50 cursor-default'>
                  Paid
                </button>
              )}
